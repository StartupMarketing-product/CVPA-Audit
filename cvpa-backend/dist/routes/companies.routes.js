...
const data_collector_service_1 = require("../services/data-collector.service");
const nlp_service_1 = require("../services/nlp.service");
const scoring_service_1 = require("../services/scoring.service");
const router = express_1.default.Router();
const dataCollector = new data_collector_service_1.DataCollectorService();
const nlpService = new nlp_service_1.NLPService();
const scoringService = new scoring_service_1.ScoringService();
// Get all companies
router.get('/', auth_1.authenticateToken, async (req, res) => {
    try {
        const result = await database_1.pool.query('SELECT * FROM companies WHERE created_by = $1 ORDER BY created_at DESC', [req.user.id]);
...
router.get('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const companyResult = await database_1.pool.query('SELECT * FROM companies WHERE id = $1 AND created_by = $2', [req.params.id, req.user.id]);
        if (companyResult.rows.length === 0) {
            return res.status(404).json({ error: 'Company not found' });
        }
        // Get latest audit score
        const scoreResult = await database_1.pool.query(`SELECT * FROM audit_scores 
       WHERE company_id = $1 
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await database_1.pool.query(`INSERT INTO companies (name, industry, website_url, description, app_store_id, google_play_package, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING *`, [name, industry, website_url || null, description || null, app_store_id || null, google_play_package || null, req.user.id]);
...
// Start audit
router.post('/:id/audits', auth_1.authenticateToken, async (req, res) => {
    try {
        const { time_period_start, time_period_end, sources } = req.body;
        const companyId = req.params.id;
        // Verify company ownership
        const companyCheck = await database_1.pool.query('SELECT id FROM companies WHERE id = $1 AND created_by = $2', [companyId, req.user.id]);
        if (companyCheck.rows.length === 0) {
            return res.status(404).json({ error: 'Company not found' });
        }
        // Create audit record
        const auditResult = await database_1.pool.query(`INSERT INTO audits (company_id, status, time_period_start, time_period_end)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [companyId, 'collecting', time_period_start, time_period_end]);
        ...
        (async () => {
            try {
                ...
                if (sources.includes('website')) {
                    const companyResult = await database_1.pool.query('SELECT website_url FROM companies WHERE id = $1', [companyId]);
                ...
                if (sources.includes('app_store')) {
                    ...
                    const companyResult = await database_1.pool.query('SELECT app_store_id FROM companies WHERE id = $1', [companyId]);
                ...
                if (sources.includes('google_play')) {
                    ...
                    const companyResult = await database_1.pool.query('SELECT google_play_package FROM companies WHERE id = $1', [companyId]);
               ...
               VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [prop.company_id, prop.source_type, prop.source_url, prop.extracted_text,
                ...
                for (const review of reviewsResult.rows) {
                    ...
                    await database_1.pool.query('UPDATE reviews SET sentiment_score = $1 WHERE id = $2', [analysis.sentiment, review.id]);
                    // Save analysis
                    await database_1.pool.query(`INSERT INTO customer_feedback_analysis 
             (company_id, review_id, jobs_mentioned, pains_mentioned, gains_mentioned, sentiment, topics)
             VALUES ($1, $2, $3, $4, $5, $6, $7)`, [companyId, review.id, JSON.stringify(analysis.jobs_mentioned),
                ...
                // Update audit status
                await database_1.pool.query('UPDATE audits SET status = $1 WHERE id = $2', ['analyzing', audit.id]);
                // Calculate scores
                await scoringService.calculateScores(companyId, audit.id);
                // Identify gaps
                await scoringService.identifyGaps(companyId, audit.id);
                // Mark audit as complete
                await database_1.pool.query('UPDATE audits SET status = $1, end_date = CURRENT_TIMESTAMP WHERE id = $2', ['completed', audit.id]);
            }
            catch (error) {
                console.error('Error in audit processing:', error);
                await database_1.pool.query('UPDATE audits SET status = $1 WHERE id = $2', ['failed', audit.id]);
            }
        })();
...
router.get('/:id/data-sources', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const reviewCounts = await database_1.pool.query(`SELECT 
        source,
        COUNT(*) as count,
...
router.get('/:id/audits', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id: companyId } = req.params;
        // Verify company ownership
        const companyCheck = await database_1.pool.query('SELECT id FROM companies WHERE id = $1 AND created_by = $2', [companyId, req.user.id]);
        ...
        const auditsResult = await database_1.pool.query('SELECT * FROM audits WHERE company_id = $1 ORDER BY created_at DESC', [companyId]);
...
router.get('/:id/audits/:auditId/detailed', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const promisesJobs = await database_1.pool.query(`SELECT * FROM value_propositions 
       WHERE company_id = $1 AND category = 'job' 
       ORDER BY confidence DESC 
       LIMIT 5`, [companyId]);
        const promisesPains = await database_1.pool.query(`SELECT * FROM value_propositions 
       WHERE company_id = $1 AND category = 'pain' 
       ORDER BY confidence DESC 
       LIMIT 5`, [companyId]);
        const promisesGains = await database_1.pool.query(`SELECT * FROM value_propositions 
       WHERE company_id = $1 AND category = 'gain' 
       ORDER BY confidence DESC 
       LIMIT 5`, [companyId]);
        // Get customer feedback analysis
        const feedbackResult = await database_1.pool.query(`SELECT cfa.*, r.review_text, r.rating, r.review_date, r.source
       ...
       WHERE cfa.company_id = $1`, [companyId]);
        ...
        const processFeedbackForDimension = (category, promises) => {
            const keyPoints = [];
...
router.get('/:id/audits/:auditId', auth_1.authenticateToken, async (req, res) => {
    try {
       ...
       WHERE cfa.company_id = $1`, [companyId]);
        ...
        const enhancedGaps = await Promise.all(gapsResult.rows.map(async (gap) => {
            ...
            if (gap.gap_type === 'jobs') {
                ...
                realityPoints = feedbackResult.rows
                    .filter(f => {
                    const jobsMentioned = (f.jobs_mentioned || []).map((j) => j.text?.toLowerCase() || '');
                    // Check if review mentions any of the promised jobs
                    const mentionsPromisedJob = promisedJobTexts.some(promiseText => jobsMentioned.some((mentioned) => mentioned.includes(promiseText.substring(0, 20)) || promiseText.includes(mentioned.substring(0, 20))));
                ...
                realityPoints = feedbackResult.rows
                    .filter(f => {
                    const painsMentioned = (f.pains_mentioned || []).map((p) => p.text?.toLowerCase() || '');
                    return painsMentioned.some((pain) => promisedPainTexts.some(promiseText => pain.includes(promiseText.substring(0, 20)) || promiseText.includes(pain.substring(0, 20))));
                })
                    .slice(0, 5)
                    .map(f => ({
                    text: f.review_text,
                ...
                realityPoints = feedbackResult.rows
                    .filter(f => {
                    const gainsMentioned = (f.gains_mentioned || []).map((g) => g.text?.toLowerCase() || '');
                    const mentionsPromisedGain = promisedGainTexts.some(promiseText => gainsMentioned.some((mentioned) => mentioned.includes(promiseText.substring(0, 20)) || promiseText.includes(mentioned.substring(0, 20))));
